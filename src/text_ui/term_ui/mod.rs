...
use text_ui::editor::Editor;
use text_ui::formatter::{block_index_and_offset, LineFormatter, LINE_BLOCK_LENGTH};
use text_ui::string_utils::{line_ending_to_str, rope_slice_is_line_ending, LineEnding};
...
impl TermUI {
        ...
        ui_loop!(
            ...
            key_press(key) {
                ...
                match key {
                    // quits program
                    Key::Ctrl('q') => {
                        self.editor.reset_send_message();
                        self.quit = true;
                    ...
                    Key::Ctrl('s') => {
                        if !self.editor.display_all {
                            self.editor.reset_send_message();
                            self.editor.send_message();
                        }
                    }

                    // opens your write message page
                    Key::Ctrl('w') => {
                        if !self.editor.display_all {
                            self.editor.reset_send_message();
                            self.editor.display_write_messages();
                        } else {
                            self.editor.reset_send_message();
                            self.editor.display_all_messages();
                        }
                    }
    ...
    fn draw_editor(
        ...
        let mut sent_message = "";
        let mut window_type = "*********** WRITE MESSAGES ***********";
        if editor.sent_msg {
            sent_message = "SENT MESSAGE !!!";
        }

        if editor.display_all {
            window_type = "*********** DISPLAY MESSAGES ***********";
        }
        let name = format!("{}{}", window_type, sent_message);

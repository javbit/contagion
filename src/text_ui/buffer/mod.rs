...
use text_ui::string_utils::char_count;
use text_ui::utils::{is_grapheme_boundary, next_grapheme_boundary, prev_grapheme_boundary, RopeGraphemes};
...
impl Buffer {
    ...
    pub fn load_received(path: &Path, receiving_channel: &mut Rx) -> Buffer {
        if let Ok(f) = OpenOptions::new().write(true)
                                     .append(true)
                                     .open(path) {
             let mut file = BufWriter::new(f);
             if let Ok(Async::Ready(bytes)) = receiving_channel.poll() {
    ...
    pub fn send_message(&mut self, path: &Path, sender_channel: &Tx) -> io::Result<()> {
        // let msg_path = PathBuf::from("sent_messages.txt");

        // save current message & write to message file
        let mut curr = BufWriter::new(try!(File::create(path)));
        // let mut msg = BufWriter::new(try!(File::create(msg_path)));
        //
        // let mut f = OpenOptions::new().write(true)
        //                              .append(true)
        //                              .open("all_messages.txt");
        //
        // let mut file = BufWriter::new(try!(f));

        let mut result = String::new();

        for c in self.text.chunks() {
            let _ = curr.write(c.as_bytes());
            // let _ = msg.write(c.as_bytes());
            // let _ = file.write(c.as_bytes());
            let mut buf = BytesMut::with_capacity(c.len());
            buf.put(c);
            let x = Bytes::from(buf);
            let _ = sender_channel.unbounded_send(x);
        }
